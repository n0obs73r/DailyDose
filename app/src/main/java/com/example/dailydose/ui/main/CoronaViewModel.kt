...
class CoronaViewModel : ViewModel() {
    ...
    private lateinit var stateCasesList: Map<String, CaseModel>
    init {
        viewModelScope.launch {
            stateCasesList = CoronaUtils.getStateCases(apiStateUrl)
        }
    }
    val stateCases = MutableLiveData<CaseModel>()
    fun getStateCases(state: String) {
        stateCases.postValue(stateCasesList.getOrDefault(state, CaseModel()))
    }
}